...
export class EmotionalIntelligence {
  ...
  private loadEmotionPatterns(): Record<string, EmotionPattern> {
    return {
      stressed: {
        ...
        patterns: [/too much/i, /can't handle/i, /falling behind/i, /so many things/i],
      ...
      anxious: {
        ...
        patterns: [/what if/i, /worried about/i, /scared that/i, /nervous about/i],
        contextClues: ["future", "unknown", "interview", "presentation", "test"]
      },
      happy: {
        keywords: ["happy", "excited", "joy", "great", "amazing", "wonderful", "thrilled"],
        patterns: [/so happy/i, /can't wait/i, /feel great/i, /love it/i],
        contextClues: ["success", "achievement", "good news", "celebration", "vacation"]
      },
      angry: {
      ...
      lonely: {
        keywords: ["lonely", "alone", "isolated", "miss", "empty", "disconnected"],
        patterns: [/feel alone/i, /no one understands/i, /miss you/i, /wish I had/i],
        contextClues: ["friends", "relationship", "family", "distance", "social"]
      },
      confused: {
        keywords: ["confused", "lost", "don't know", "uncertain", "unclear", "mixed up"],
        patterns: [/don't understand/i, /not sure/i, /confused about/i, /don't know what/i],
  ...
  analyzeEmotionalState(text: string, context?: Record<string, unknown>): EmotionalState {
    ...
    for (const [emotion, pattern] of Object.entries(this.emotionPatterns)) {
      ...
      if (textLower.includes("so ") || textLower.includes("really ") || textLower.includes("very ")) {
        score += 0.2;
      }
  ...
  private calculateEmotionalStability(emotions: EmotionalState[]): string {
    ...
    const intensityVariation = this.calculateVariation(emotions.map(e => e.intensity));
    const emotionChanges = this.countEmotionChanges(emotions);

    if (intensityVariation < 0.2 && emotionChanges < emotions.length * 0.3) {
      return "stable";
    } else if (intensityVariation > 0.5 || emotionChanges > emotions.length * 0.7) {
      return "volatile";
    } else {
      return "moderate";
    }
